...
import { BrainCircuit, X as CloseIcon, History, Settings, Check } from 'lucide-react';
...
export default function ExecutiveSummary() {
    ...
    const loadAIContext = async () => {
        ...
        try {
            const res = await fetch('/api/ai/context?role=executive');
    ...
    const saveAIContext = async () => {
        ...
        try {
            await fetch('/api/ai/context', {
                ...
                body: JSON.stringify({ role: 'executive', content: aiContext })
            });
                ...
                >
                    <div style={{ marginTop: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{ flex: 1, height: '8px', background: 'var(--bg-tertiary)', borderRadius: '4px', overflow: 'hidden' }}>
                            <div style={{
                                width: `${Math.min(((data?.mtdRevenue || 0) / 3300000) * 100, 100)}%`,
                                height: '100%',
                                background: (data?.mtdRevenue || 0) >= 3300000 ? 'var(--success, #10b981)' : ((data?.mtdRevenue || 0) >= 2600000 ? '#f59e0b' : 'var(--danger, #ef4444)'),
                                borderRadius: '4px',
                    ...
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)', marginTop: '8px' }}>
                        {(data?.mtdRevenue || 0) >= 3300000
                            ? '✅ ¡Meta de ventas superada!'
                            : `Faltan ${formatCurrency(3300000 - (data?.mtdRevenue || 0))} para meta`}
                    </div>
                </Card>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', gap: '16px' }}>
                    ...
                    <button
                        ...
                        title="Configurar Reglas del CEO (IA)"
                        style={{
                            padding: '10px 18px',
                            borderRadius: '30px',
                            background: 'var(--bg-tertiary)',
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                <div>
                                    <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 800 }}>Reglas y Contexto del CEO</h3>
                                    <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-tertiary)' }}>Añade reglas de negocio ejecutivas.</p>
                                </div>
                            </div>
                        ...
                        {loadingContext ? (
                            ...
                            <textarea
                                ...
                                placeholder="Ej: Analiza esto desde la perspectiva de expansión a un nuevo mercado..."
                                style={{
